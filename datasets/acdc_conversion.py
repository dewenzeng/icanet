...
if __name__ == "__main__":
    ...
    parser.add_argument("--folder_train", help="folder where the training data is", type=str)
    parser.add_argument("--folder_test", help="folder where the testing data is", type=str)
    parser.add_argument("--folder_out", help="folder where the converted data is", type=str)
    args = parser.parse_args()
    folder = args.folder_train
    ...
    json_dict['training'] = [{'image': "./imagesTr/%s.nii.gz" % i.split("/")[-1][:-12], "label": "./labelsTr/%s.nii.gz" % i.split("/")[-1][:-12]} for i in
                             all_train_files]
    json_dict['test'] = [{'image': "./imagesTs/%s.nii.gz" % i.split("/")[-1][:-12], 'label': "./labelsTs/%s.nii.gz" % i.split("/")[-1][:-12]} for i in all_test_files]
